...
// ═══════════════════════════════════════════════════════════════════
//  UPDATE DATA — triggers the GitHub Actions workflow_dispatch event
// ═══════════════════════════════════════════════════════════════════

function ghToken()     { return localStorage.getItem('gh_token') || ''; }
function saveGhToken(t){ localStorage.setItem('gh_token', t.trim()); }
...
async function dispatchWorkflow(token) {
  ...
  if (btn) { btn.disabled = true; btn.innerHTML = '<span>⏳</span> Triggering…'; }

  try {
    const resp = await fetch(
      `https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/actions/workflows/update_data.yml/dispatches`,
      {
        method: 'POST',
        headers: {
    ...
    if (resp.status === 204) {
      showToast('Data update triggered! New data will be ready in ~10 minutes.', 'success');
      if (btn) {
        btn.innerHTML = '<span>✅</span> Triggered!';
        setTimeout(() => { btn.innerHTML = '<span>🔄</span> Update Data'; btn.disabled = false; }, 4000);
      }
    } else if (resp.status === 401 || resp.status === 403) {
      saveGhToken('');
      showToast('Token invalid or missing "workflow" scope — please enter a new token.', 'error');
      if (btn) { btn.innerHTML = '<span>🔄</span> Update Data'; btn.disabled = false; }
      showTokenModal();
    } else if (resp.status === 404) {
      showToast('Workflow not found — make sure update_data.yml is committed to the repo.', 'error');
      if (btn) { btn.innerHTML = '<span>🔄</span> Update Data'; btn.disabled = false; }
    } else {
      const text = await resp.text().catch(() => '');
      showToast(`GitHub error ${resp.status}: ${text.slice(0, 120)}`, 'error');
      if (btn) { btn.innerHTML = '<span>🔄</span> Update Data'; btn.disabled = false; }
    }
  } catch (e) {
...
function showTokenModal() {
  ...
  if (modal) {
    ...
    if (inp) { inp.value = ghToken(); inp.focus(); }
  }
}
...
function tokenBackdropClick(e) { if (e.target === e.currentTarget) closeTokenModal(); }

function saveTokenAndDispatch() {
  const inp = document.getElementById('token-input');
  const tok = inp?.value?.trim();
  if (!tok || !tok.startsWith('ghp_')) {
    showToast('Please paste a valid GitHub token (starts with ghp_)', 'error');
    return;
  }
  saveGhToken(tok);
  closeTokenModal();
  dispatchWorkflow(tok);
}
